...
shinyServer(function(input, output, session) {
  ...
  shots = reactive({
    ...
    use_default_shots = current_player()$person_id == default_player$person_id & current_season() == default_season & minutes() == 5

    if (use_default_shots) {
      default_shots
    } else {
      minutes = input$minutes
      fetch_shots_by_player_id_and_season(current_player()$person_id, current_season(), minutes)
    }
  })
